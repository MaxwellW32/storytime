...
export default function ViewStory({ fullData }: { fullData: StoryData }) {
    ...
    const [canShowAudioControls, canShowAudioControlsSet] = useState(false)
    ...
    useEffect(() => {
        if (reading) {
            readingTimer.current = setTimeout(() => {
                saveRecentlySeenStories(fullData.storyid)
            }, 150000)
        } else {
                ...
                <div className={`${styles.readingArea} niceScrollbar`} style={{ gridTemplateColumns: gameModesShowing ? "1fr 1fr" : "1fr" }}>
                    <div>
                        <div style={{ position: "absolute", right: 0, margin: "1rem", display: "flex", alignItems: "center", gap: "1rem" }}>
                            <div style={{ display: canShowAudioControls ? "none" : "block" }}>
                                <ReactPlayer
                                    loop={true}
                                    ...
                                    width={50}
                                    onPlay={() => {
                                        canPlayAudioSet(true)
                                        canShowAudioControlsSet(true)
                                    }}
                                />
                            </div>

                            {canShowAudioControls && <div style={{ width: "1rem", aspectRatio: "1/1", fill: "var(--textColor)" }} onClick={() => { canPlayAudioSet(prev => !prev) }}>
                                {canPlayAudio ? (
                                    <>
